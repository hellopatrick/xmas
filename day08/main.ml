...
type t = {hints: CharSet.t list; output: CharSet.t list}

let to_charset str = str |> String.to_list |> CharSet.of_list

let parse_line l =
  Scanf.sscanf l "%s %s %s %s %s %s %s %s %s %s | %s %s %s %s"
...
let decode {hints; output} =
  ...
  in
  let deduced =
    CharSet.of_list [bottom_left; bottom_right; top; top_right; top_left; middle]
  in
